  ...
  class Home extends BaseController
  {
      ...
      public function productListing(){
         ...
         foreach ($products as $key => $value) {
             ...
             $content.="</td>";
             $content.="<td>";
             $content.=number_format($value->unitPrice)."*";
             $content.='<span class="bold red1">';
             $content.=$value->count.'</span>';
             $content.="</br>";
             $content.='<span class="bold">';
             $content.=number_format((float)$value->pprice);
             $content.='</span>';
             $content.="</td>";
             $content .="<td>";
              ...
              if($de[0]->get == "buy"){
                ...
                foreach($pros as $vals){
                  ...
                  if(!empty($add_options) && $add_options!=NULL && sizeof($add_options) > 0){
                    foreach ($add_options as $key_add => $value_add) {
                      $this->base_model->plusValue("tbl_product_option","count",$vals->count,array(array("pcode",$vals->rid)),"-");
                      $this->base_model->plusValue("tbl_product_option","salecount",$vals->count,array(array("pcode",$vals->rid)),"+");
                    }
                  }
    ...
    public function company(){
      $data['company'] = $this->base_model->getCompany();
    ...
    $this->base_model->deleteRecordsById("tbl_purchasedproduct",$ORD_SEQ);
    $this->base_model->insertPurchase($theader,$ORD_SEQ);
    redirect("/");
  }
  ...
  public function updateTrackNumber(){
    ...
    public function saveFooter(){
      $this->base_model->updateDataById(77,array("description"=>$this->input->post("content"),"type"=>21),"banner","id"); 
      if($_FILES['image']['name'] !=""){
  ...
  public function shopProducts(){
    ...
    $records_count = sizeof($this->base_model->getSelect("tbl_sproducts",$where,array(array("record"=>"updated_date","value"=>"DESC"))));
    $returns = $this->paginationCompress ( "shopProducts/", $records_count, $shPageSize);
    $data['products'] = $this->base_model->getSelect("tbl_sproducts",$where,array(array("record"=>"p_idx","value"=>"ASC")),
                                                    null,array(array("record"=>$returns["page"],"value"=>$returns["segment"])));
  ...
  public function getCateogrys(){
    ...
    $this->base_model->deleteRecordCustom("tbl_purchasedproduct","delivery_id",$id);
  ...
  public function Comment_W(){
    ...
    $sOrdSeq = $this->input->get("sOrdSeq");
    $data['map']= directory_map('../upload/silsa/'.$sOrdSeq, FALSE, TRUE);
    $data['comments'] = $this->base_model->getSelect("tbl_delivery_comment",array(array("record"=>"delivery_id","value"=>$sOrdSeq)));
  ...
  public function verifyDeposit(){
    ...
    foreach($history as $value) {
      if($value->type ==4){
        ...
        if($value->type == 60) 
        { 
          $state = 7;$ins = array("state"=>$state,"payed_send"=>1,"payed_checked"=>1,"pays"=>0);
        }
        $this->base_model->updateDataById($value->delivery_id,$ins,"delivery","id");
      }
    }
    if(!empty($history)){
      $am = (int)($history[0]->pamount/10000);
      $ed = $this->base_model->getSelect("tbl_point", array(  array("record"=>"type","value"=>2),
                                                              array("record"=>"from_gold <=","value"=>$am )),
                              array(array("record"=>"from_gold","value"=>"DESC")));
      if(!empty($ed)){
        foreach ($ed as $key => $valuep) {
          $tt = $valuep->point_type;
          $tp = $valuep->point;
          if($tt==2)
            $tp = (int)$history[0]->pamount*$tp/100;
          $this->base_model->plusValue("tbl_users","point",$tp,array(array("userId",$userId)),"+");
          $ss = $this->base_model->getSelect("tbl_users",array(array("record"=>"userId","value"=>$userId)));
          $this->base_model->insertArrayData("tbl_point_users",array( "userId"=>$userId,
                                                                      "point_id"=>$valuep->id,
                                                                      "point"=>"+".$tp,
                                                                      "type"=>$valuep->type,
                                                                      "remain"=>$ss[0]->point));
  ...
  public function cancelDeposit(){
    ...
    if(!empty($history)){
      foreach($history as $value) {
        ...
        if($value->point > 0){
          $this->base_model->plusValue("tbl_users","point",$value->point,array(array("userId",$value->userId)),"+");
          $ss = $this->base_model->getSelect("tbl_users",array(array("record"=>"userId","value"=>$value->userId)));
          $this->base_model->insertArrayData("tbl_point_users",array( "userId"=>$value->userId,
                                                                      "point"=>"+".$value->point,
                                                                      "type"=>6,
                                                                      "remain"=>$ss[0]->point));
        ...
        if($value->type!=4) {
          ...
          if($value->type ==2) $ss=array("state"=>4,"payed_checked"=>0,"pays"=>0);
          if($value->type ==3) $ss=array("state"=>19,"return_check"=>0,"pays"=>0);
          if($value->type ==60) $ss=array("state"=>5,"payed_checked"=>0,"payed_send"=>0,"pays"=>0);
  ...
  public function Acting_D(){
    ...
    echo "<script>
              window.location.href='/admin/dashboard?step=11';
            </script>";
  }
  ...
  public function addPage(){
    ...
    if($return > 0 ){
      if (!file_exists("../upload/banner/".$return."/"))
        mkdir("../upload/banner/".$return."/", 0777);
        $this->load->library('upload',$this->set_upload_options("../upload/banner/".$return."/",30000,false));
        $this->upload->initialize($this->set_upload_options("../upload/banner/".$return."/",30000,false));
        $count = sizeof($_FILES['banner_image']['name']);
        for($i=0;$i<$count;$i++){
  ...
  public function Mem_U(){
    $sKind = $this->input->post("sKind");
    $chkMemCode = $this->input->post("chkMemCode");
    $sMemLvlChk =  $this->input->post("sMemLvlChk");
    if($sKind =="U"){
      session_start();
      $my_session_id = session_id();
      session_write_close();
      foreach($chkMemCode as $value){
        $this->base_model->updateDataById($value,array("roleId"=>$sMemLvlChk),"tbl_users","userId");
  ...
  public function shopcategory(){
    $data['first_category'] = $this->base_model->getSelect("tbl_leftcategory",array(array("record"=>"parent",'value'=>0)));
  ...
  public function getshopcategory(){
    ...
    if($type =="list")
    { 
      $c = $this->base_model->getSelect("tbl_leftcategory",array(array("record"=>"parent","value"=>$id)));
  ...
  public function product_wish(){
    ...
    $p_newview = $this->input->get("p_saleview");
    $p_bestview = $this->input->get("p_saleview");
  ...
  public function updateProductTalk(){
    ...
    redirect("product_talk_modify?type=".$type."&mode=".$mode."&status=".$status."&id=".$return);
  }
}    
?>
